...
class Blogclass extends CI_Model {
    ...
    function inserisci_messaggio($lastid, $array) {
        ...
        foreach ($array as $arr) {
            ...
            $this->db->query('UPDATE ' . $this->config->item('prefix') . 'blog SET lang_' . $arr . ' = ' . $id . ' WHERE id_blog = ' . $lastid . '');
        }
    }
    ...
    function inserisci_messaggiocat($lastid, $array) {
        ...
        foreach ($array as $arr) {
            ...
            $query = $this->db->insert($this->config->item('prefix') . 'messaggi', $data);
            $id = $this->db->insert_id();
            $this->db->query('UPDATE ' . $this->config->item('prefix') . 'blog_cat SET lang_' . $arr . ' = ' . $id . ' WHERE id_blogcat = ' . $lastid . '');
        }
    }
    ...
    function modifica_messaggio($lastid, $array) {
        ...
        foreach ($array as $arr) {
            ...
            $query = $this->db->query('SELECT lang_' . $arr . ' FROM ' . $this->config->item('prefix') . 'blog WHERE id_blog = ' . $lastid . '');
            $row = $query->row();
            $suffisso = 'lang_' . $arr;
    ...
    function modifica_messaggiocat($lastid, $array) {
        ...
        foreach ($array as $arr) {
            ...
            $query = $this->db->query('SELECT lang_' . $arr . ' FROM ' . $this->config->item('prefix') . 'blog_cat WHERE id_blogcat = ' . $lastid . '');
            $row = $query->row();
            $suffisso = 'lang_' . $arr;
            $progressivo = $row->$suffisso;
            $data->messaggio = $this->input->post($arr);
    ...
    function inserisci_blog($data_img) {
        ...
        $risultato = $this->db->insert($this->config->item('prefix') . 'blog', $this);
        ...
        $tags = explode(',', $tags);
        foreach ($tags as $tag) {
            $instag->id_blog = $lastid;
            $instag->tag = $tag;
            $this->db->insert($this->config->item('prefix') . 'blog_tags', $instag);
        }
    ...
    function lista_blog() {
        ...
        $query = $this->db->get($this->config->item('prefix') . 'blog');
        $c = 0;
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                ...
                if ($elem[$c]->id_cat != 0) {
                    $query2 = $this->db->get_where($this->config->item('prefix') . 'blog_cat', Array('id_blogcat' => $elem[$c]->id_cat));
                    $row2 = $query2->row();

                    $elem[$c]->cat_blog = $row2->nome;
                } else {
                    $elem[$c]->cat_blog = "";
                }
                $c++;
    ...
    function dati_catblog($id, $inc_traduzioni = 0) {



        $query = $this->db->query('SELECT * FROM ' . $this->config->item('prefix') . 'blog_cat WHERE id_blogcat = ' . $id);
        //return $query->result();
        $c = 0;
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
    ...
    function dati_blog($id, $inc_traduzioni = 0) {


        $query = $this->db->query('SELECT *,(SELECT GROUP_CONCAT(tag) FROM ' . $this->config->item('prefix') . 'blog_tags WHERE id_blog = ' . $this->config->item('prefix') . 'blog.id_blog) tags FROM ' . $this->config->item('prefix') . 'blog WHERE id_blog = ' . $id);
        //return $query->result();
        $c = 0;
        foreach ($query->result() as $row) {
            ...
            if ($inc_traduzioni == 1) {
                $elem[$c]->titolo_traduzioni = $this->getBlogtraduzioni($elem[$c]->lang_titolo);
                $elem[$c]->sottotitolo_traduzioni = $this->getBlogtraduzioni($elem[$c]->lang_sottotitolo);
                $elem[$c]->testo_traduzioni = $this->getBlogtraduzioni($elem[$c]->lang_testo);
            }
        }
        return $elem;
    }

    function getBlogtraduzioni($id) {

        $query = $this->db->get_where($this->config->item('prefix') . 'messaggi', Array('progressivo' => $id));
        return $query->result();
    }

    function modifica_blog($id, $data_img) {
        ...
        $risultato = $this->db->update($this->config->item('prefix') . 'blog', $this, array('id_blog' => $id));
        ...
        $this->db->delete($this->config->item('prefix') . 'blog_tags', array('id_blog' => $id));
        /* reinserisco i tag */
        $tags = $this->input->post('tags');
        $tags = explode(',', $tags);
        foreach ($tags as $tag) {
    ...
    function cancella_blog($id) {

        return $this->db->delete($this->config->item('prefix') . 'blog', array('id_blog' => $id));
    }

    function lista_catblog() {

        $query = $this->db->get($this->config->item('prefix') . 'blog_cat');
        return $query->result();
    }

    function inserisci_catblog() {
        ...
        $risultato = $this->db->insert($this->config->item('prefix') . 'blog_cat', $this);
        $lastid = $this->db->insert_id();
    ...
    function modifica_catblog($id) {

        $this->nome = $this->input->post('nome'); // please read the below note
        $this->cat_rewrite = $this->input->post('rewrite');
        $this->cat_rewrite = url_title($this->cat_rewrite, 'underscore', TRUE);

        $risultato = $this->db->update($this->config->item('prefix') . 'blog_cat', $this, array('id_blogcat' => $id));
    ...
    function cancella_blogcat($id) {

        return $this->db->delete($this->config->item('prefix') . 'blog_cat', array('id_blogcat' => $id));
    }
